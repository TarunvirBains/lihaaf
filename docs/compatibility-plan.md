...
- `--package <pkg>` / `-p <pkg>` — workspace-member package selector. Required when `--compat-root` points at a workspace root (a `Cargo.toml` declaring `[workspace]` without `[package]`); see §3.2.3 ("Workspace-member entry via `--package`") for the resolver semantics. Mutually exclusive with `--compat-manifest` (which supplies an explicit manifest path, bypassing the resolver).
...
**Resolution: compat-mode normalization convergence.** Lihaaf's normalizer gains a compat-mode flag (`NormalizationContext::compat_short_cargo`) that emits Trybuild's shorter `$CARGO/<crate>-<ver>/<rest>` form. The compat driver sets the flag. Non-compat lihaaf ALSO collapses the registry index hash (the `<host>-<16hex>` segment, for the `index.crates.io` and `github.com` hosts) to the stable placeholder token `$CARGO_HASH` so registry-path snapshots are portable across machines — the non-compat output keeps the `$CARGO/registry/src/.../` shape but with the volatile per-machine hash replaced (`$CARGO/registry/src/$CARGO_HASH/<crate>-<ver>/<rest>`). It is therefore no longer byte-identical to the prior v0.1 normalizer on registry paths that carried a hash; that one-time change is called out in the CHANGELOG. With the compat flag set, all four placeholders converge on the same **shape** between the two normalizers, and the comparison is direct — no translation table, no runtime lookup of the registry index hash, no compat-only translator module. Ordering note: in compat mode the `compat_short_cargo` rewrite (composition step 5) runs BEFORE foreign-span suppression (composition step 7), so a shortened `$CARGO/<crate>-<ver>/src/lib.rs:3:1` pointer is correctly detected as a `$CARGO/` foreign pointer by the suppression stage and has its `:LINE:COL` tail stripped and any following body suppressed per the standard rule. The two stages compose cleanly because suppression keys on the already-shortened `$CARGO/` prefix.
...
**Foreign-span body suppression is a deliberate lihaaf-only normalization in compat mode.** In compat mode, lihaaf suppresses foreign span bodies — for BOTH primary `-->` and secondary `:::` spans whose pointer resolves to a non-fixture placeholder (`$RUST` / `$CARGO` / `$WORKSPACE`) — to a kind-matched `$*_SRC` placeholder, and strips the foreign pointer's `:LINE:COL` tail (also on both marker kinds), as a core behavior; but trybuild does NOT — trybuild only blanks the gutter line numbers and keeps the source text. So for any fixture that triggers a foreign span (primary OR secondary), the compat comparison envelope will show a trybuild-vs-lihaaf difference (a `snapshot_mismatch` entry). This is INTENTIONAL: the compat report correctly surfaces the new normalization behavior as a diff rather than hiding it. Adopters migrating from trybuild who see `$RUST_SRC` / `$CARGO_SRC` / `$WORKSPACE_SRC` (or a tail-stripped foreign pointer, on either a `-->` or `:::` line) in the lihaaf column of the compat report can confirm these are normalized foreign span bodies, not regressions. (Cross-ref: the §9.2 "What 'agreement' means" carve-out.)
...
**Implementation surface.** Lihaaf's existing normalizer (`src/normalize.rs`) substitutes `$CARGO/registry` via a literal-prefix `push_path` call (line 83) executed inside the `replace_advancing` loop (line 116). In NON-COMPAT mode the literal-prefix substitution is retained for the `$CARGO/registry` PREFIX, and the registry index hash that follows it (the `<host>-<16hex>` segment) is collapsed to the stable placeholder token `$CARGO_HASH`, leaving `$CARGO/registry/src/$CARGO_HASH/<crate>-<ver>/<rest>` (see the secondary-span normalization work). In COMPAT mode the `$CARGO/registry` literal prefix is NOT pushed (it is gated off); the existing `rewrite_cargo_short` post-pass instead matches the RAW `<cargo_registry>/src/<host>-<hash>/` segment and strips it entirely to the short form `$CARGO/<crate>-<ver>/<rest>`, so `$CARGO_HASH` does NOT appear in compat output. The two modes do NOT scan the same input bytes — compat operates on the raw filesystem registry path as a post-pass, non-compat operates within the `$CARGO/registry` prefix substitution — but they share the same hash-RECOGNITION shape (`<host>-<16 lowercase hex>` followed by `/`, for the `index.crates.io` and `github.com` hosts) and the same structural rejection rules; they differ only in whether the `registry/src/.../` shell is kept (non-compat) or stripped (compat). Compat-mode adds:

1. A new `compat_short_cargo: bool` field on `NormalizationContext`, default `false`. Non-compat callers leave it `false`; their registry-path output keeps the `$CARGO/registry/src/.../` shape but with the volatile `<host>-<16hex>` index hash collapsed to the stable placeholder token `$CARGO_HASH` (the cross-machine-portability fix), so it is byte-identical to other machines' non-compat output but NOT to the prior v0.1 normalizer on paths that contained a registry hash.
...
**Scope and tests.** Roughly ~30–50 lines of Rust (the helper + the bool field + the selection branch + unit tests). Tests under `src/normalize.rs` assert: (a) compat-mode output for an `index.crates.io-<hash>` input matches Trybuild's `$CARGO/<crate>-<ver>/<rest>` form; (b) the older `github.com-<hash>` form is handled identically; (c) a path with no registry segment is left unchanged; (d) with the flag `false`, the `<host>-<16hex>` index hash is collapsed to the stable placeholder token `$CARGO_HASH` (so `$CARGO/registry/src/index.crates.io-<hash>/<crate>-<ver>/<rest>` becomes `$CARGO/registry/src/$CARGO_HASH/<crate>-<ver>/<rest>`) while the `$CARGO/registry/src/.../` shell is retained, and two machines with different registry hashes produce identical non-compat output. The hash collapse recognizes the two cargo-used registry hosts (`index.crates.io` and `github.com`) only — alternate or private registry hosts are not currently supported. No new top-level module; no regex.
...
#### §3.2.3.1 Option H self-patch policy (4-rule table)

When the upstream Cargo.toml contains `[patch.crates-io]` entries relevant to the crate-under-test, lihaaf applies the following four rules in order:

- **Rule 1 (INJECT):** If your upstream Cargo.toml has no `[patch.crates-io]` entry for the crate-under-test, lihaaf injects one pointing at the staged overlay directory (`<upstream>/target/lihaaf-overlay/`). This covers two upstream shapes: **serde-json-shape**, where a workspace member or path-dep references the crate-under-test by its registry name (`<crate> = "X.Y"`) — without injection, cargo's resolver sees the staged-overlay package and the registry-resolved package as two competing sources and fails with `ambiguous specification`; and **anyhow-shape**, where no in-graph entity back-references the crate by registry name — injection is benign here (cargo collapses the unused patch), and lihaaf applies it precautionarily so every Rule-1 overlay carries the same shape regardless of upstream dep-graph topology.

- **Rule 2 (REMAP):** If your upstream has `[patch.crates-io.<crate>] = { path = "." }` (or any path-form that resolves to the upstream root crate), lihaaf rewrites the entry to point at the staged overlay directory. cargo's resolver re-anchors `.` relative to the staged overlay manifest, but lihaaf emits the absolute form for determinism and clarity. The semantic intent (`patch crates-io.<crate> with the equivalent-to-root path-source`) is preserved. This covers cxx-shape pilots that carry `[patch.crates-io.cxx] = { path = "." }` in their upstream `Cargo.toml`; without REMAP, the resolver sees two competing sources (`links = "cxxbridge1"` collision).
...
**Why REMAP rather than preserve-as-is.** cargo anchors `[patch.crates-io.X].path` relative to the manifest DECLARING the patch, not the manifest the path was copied from. If lihaaf preserved cxx's `path = "."` verbatim in the staged overlay manifest, cargo would re-anchor `.` against the staged overlay dir — which happens to work for cxx (the staged overlay dir IS the overlay root), but the reasoning is accidental and the produced overlay would contain a relative path in violation of the absolutization contract. lihaaf emits the absolute form explicitly to prevent confusion and guarantee determinism.

**Known limitation — symlinked compat-roots.** Ensure your `--compat-root` argument and your upstream's `[patch.crates-io]` entries use the same form (real path or symlinked path); otherwise Rule 2's lexical path comparison may fall to Rule 4 REJECT. The lexical normalizer collapses `//` repeated separators on Unix but does NOT resolve symlinks — if your upstream `[patch]` path and your `--compat-root` path refer to the same directory via different symlink chains, they compare unequal at the lexical layer.
...
**Workspace-member entry via `--package`.** When the adopter's target crate is a workspace MEMBER (e.g. axum-macros inside the tokio-rs/axum workspace), compat mode resolves the entry shape via `--compat-root <workspace-root>` + `--package <member-pkg>`. The resolver reads the workspace root's `[workspace.members]` array, expands globs against the workspace-root directory (`crates/*`, `axum-*`, explicit nested literals like `crates/foo`; deep globs `**` and parent traversal `..` are rejected with directed diagnostics), and matches a member by its declared `[package].name`.
...
The Option H 4-rule self-patch policy composes with the carry-down in **root-first, member-second** order: the effective `[patch.crates-io]` table is computed from the workspace root (absolutized against workspace_root), then Option H's 4 rules run against the merged table with the MEMBER as the "self" target. The 4-rule policy applies only to `[patch.crates-io]` — non-crates-io registry tables are carried verbatim (path-absolutized) without additional self-patch logic. A member-level `[patch.<registry>]` of any registry is REJECTed (cargo itself errors on member-level `[patch]`; the resolver matches by surfacing a directed diagnostic naming the offending registry key).

The baseline `cargo test` invocation runs at the workspace ROOT, not the member dir; cargo writes `Cargo.lock` at the workspace root and the baseline must find it there. Adopters who want to compare only the target member's pass/fail must override the baseline argv: `--compat-cargo-test-argv '["cargo","test","-p","<pkg>"]'` — the compat driver does NOT inject `-p <pkg>` automatically (so adopters can choose a workspace-wide baseline if they prefer).

**v0.1.0 scope: virtual workspaces only.** The workspace root manifest must declare `[workspace]` WITHOUT `[package]`. The package+workspace shape (root carries both `[package]` and `[workspace]`, e.g. some smaller multi-crate repos) is left to v0.2 / v1.0 — the resolver REJECTs that shape with a directed diagnostic naming the v0.1.0 scope.
...
**One-time `N_<crate>` baseline migration for the v0.2 foreign-span normalization.** The v0.2 foreign-span body suppression (and the foreign pointer-tail strip) is an intentional lihaaf-only normalization that trybuild does not apply, so any pilot fixture quoting a foreign (`$RUST` / `$CARGO` / `$WORKSPACE`) primary or secondary span — or carrying a foreign pointer tail — produces a `snapshot_mismatch` against the trybuild reference and increments `results.mismatch_count`. These diffs are CORRECT behavior (§3.2.2 foreign-span note, the §9.2 agreement carve-out), not harness bugs. As a ONE-TIME exception to the decrement-only rule above, the v0.2 release MAY re-freeze the affected `N_<crate>` values in `compat/baseline.toml` UPWARD to the new stable `mismatch_count` each pilot's first post-v0.2 stable run produces, PROVIDED the increase is fully accounted for by intentional foreign-span suppression diffs (verified fixture-by-fixture: every newly-counted mismatch must be a foreign-span normalization diff, not a regression) and recorded in `compat/KNOWN_DIFFS.md` with the per-fixture rationale. After this one-time re-freeze, the decrement-only convention resumes: subsequent PRs may only lower the re-frozen `N_<crate>`. (Alternatively, where a pilot's affected fixtures are few and individually enumerable, they MAY instead be listed in `excluded_fixtures` — §5 gate item, lines 294/434 — so the count delta is accounted for without raising `N_<crate>`; the implementer picks per pilot whichever is cleaner, baseline re-freeze for a broad sweep or `excluded_fixtures` for a handful, and records the choice in `KNOWN_DIFFS.md`.)
...
2. Classify mismatch:
   ...
   - normalization mismatch
     When a normalization mismatch is an INTENTIONAL lihaaf-only normalization (e.g. the v0.2 foreign-span body suppression / pointer-tail strip, which trybuild does not apply), it is NOT a harness bug — do NOT "fix in lihaaf first". Instead record it in `compat/KNOWN_DIFFS.md` as an expected divergence and account for its `mismatch_count` contribution via the one-time `N_<crate>` baseline migration (or `excluded_fixtures`) described in §5. Verify fixture-by-fixture that each such diff is genuinely the intentional normalization (a foreign-span diff) and not a real regression masquerading as one before accepting it into the baseline.
