...
- **Rule 1 (INJECT):** If your upstream Cargo.toml has no `[patch.crates-io]` entry for the crate-under-test, lihaaf injects one pointing at the staged overlay directory (`<upstream>/target/lihaaf-overlay/`). This is the common case for serde-json-shape and anyhow-shape pilots (no self-patch in upstream); without injection, cargo's resolver sees two competing sources for the same package name and fails with `ambiguous specification`.
