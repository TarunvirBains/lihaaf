...
2. Every path-bearing key absolutized against the upstream crate dir. Cargo resolves path-bearing keys against the manifest's parent directory; the staged overlay lives two directories deeper than the upstream `Cargo.toml`, so relative paths must be rewritten to absolute (forward-slash) form before serialization. The covered key set is `[lib] path`, `[[bin]] path`, `[[example]] path`, `[[test]] path`, `[[bench]] path`, `[dependencies.X] path`, `[dev-dependencies.X] path`, `[build-dependencies.X] path`, `[target.*.<deps>] path`, `[workspace] members`, `[workspace] exclude`, and `[package] build`. The `[lib] path` is unconditionally injected to point at the upstream `src/lib.rs` so cargo's auto-discovery does not search the (empty) staged dir. `[package] build` is injected when the upstream carries a `build.rs` so the build script still compiles correctly under the overlay. Auto-discovery for `[[bin]]`, `[[example]]`, `[[test]]`, `[[bench]]` is explicitly disabled (`autobins = false`, etc.) because the staged dir has no `src/bin/` / `tests/` / `examples/` / `benches/` to discover from.
