...
- This plan is currently at R3 awaiting adversarial review (R1 returned Codex `VERDICT: BLOCK` with a 30-item class-enumeration sweep; R2 closed 5/6 R1 BLOCKs; Codex R2 returned `VERDICT: BLOCK` with BLOCK-3 + 3 FIX_BEFORE_IMPL + B-2 leftover — R3 addresses all of these in a surgical no-source-change cleanup on 2026-05-18). Codex xhigh (or equivalent) ALLOW required before implementer dispatch.

---

# Plan: lihaaf #53 — workspace-MEMBER subdirectory entry (`-p <package>`)

Revision: R3 (2026-05-18, post-Codex-R2-BLOCK surgical cleanup; R2 was post-Codex-R1-BLOCK + class-enumeration sweep)
...
- **R3 (2026-05-18, post-Codex-R2-BLOCK)** — R2 closed 5 of 6 R1 BLOCKs successfully. Codex R2 returned `VERDICT: BLOCK` with BLOCK-3 (Option H composition order — internal inconsistency between §5.3.bis authoritative statement and stale text elsewhere in the plan) + 3 FIX_BEFORE_IMPL findings (H-1 stale `ResolvedMember` references; H-2 stale `compat/baseline.toml` references; H-3 §10b duplicate row numbers) + B-2 leftover (`[workspace.package.include]` / `[workspace.package.exclude]` omission in §3.2.bis). R3 is a SURGICAL cleanup pass — no new sections, no structural changes, no re-litigation of R2's closed BLOCKs (§3.1.bis, §3.2.bis 4-column table structure, §4.3 glob/path/exclude resolver, §7.2 test plan T-24 through T-41). R3 deltas:

  - **BLOCK-3 fix (Option H composition order — 6 sites).** Aligned 5 stale "member-first" or "layered-after" text sites + 1 test expectation to match §5.3.bis as the single source of truth (root-first, member-second). Sites: line 697 (§5.3 bullet list — rewrote member-first phrasing to root-first with explicit §5.3.bis cross-reference); line 724 (§5.3.bis intro — strengthened to explicitly cite "root-first, member-second" as authoritative); line 739 (§5.3.bis Step 3 — clarified ordering relative to Step 1 and Rule 2's overlay-root-relative output); line 741 (§5.3.bis Rule 2 REMAP — re-emphasized overlay-root-relative output is the key semantic); line 824 (§6.2 — rewrote stale member-first wording to root-first with §5.3.bis cross-reference); line 1011 (§7.2 test #27 `option_h_root_first_member_second_with_workspace_root_self_patch_entry` — fixed assertion to match Rule 2 REMAP: final path is overlay-root-remapped to staged-overlay-dir, NOT workspace-root-relative, since `pkg-name` IS the self target).
  - **H-1 fix (stale `ResolvedMember` references — 5 sites).** R2 consolidated the R1 `ResolvedMember` struct + `UpstreamManifest` enum into the `DualRoot` struct with `Option<WorkspaceMemberContext>`. R3 replaces 5 remaining `ResolvedMember` references with the consolidated types: line 602 (§4.6 R2 refinement paragraph — reframed as "R2 type-shape contract" and moved the R1-superseded historical mention out of the body prose, with the consolidation rationale linked to the top-of-plan revision history); line 646 (§4.7 resolver tests — updated to `WorkspaceMemberContext.workspace_root_value`); line 976 (§7.2 test #10 description — updated to reference the resolver's tuple-return + driver's `DualRoot.workspace_member_context` wrap); line 1448 (§10b mirror table row 10 description — updated to match the new wording); line 1615 (§Open items closure list — relabeled "ResolvedMember struct vs tuple" / "UpstreamManifest enum vs flat struct" as descriptive "Resolver-return-shape struct vs tuple" / "Upstream-manifest enum vs flat struct" R1 open items, with the original R1 type names noted as superseded).
  - **H-2 fix (stale `compat/baseline.toml` references — 4 sites).** R2 corrected §7.4 to use the real corpus path `tests/compat/overlay_corpus/<name>.input.toml` + `<name>.expected.toml`, but 4 other sites still cited the old (invented) `compat/baseline.toml`. R3 updates: line 73 (acceptance criteria 3 — replaced with real corpus path + explicit "Out of scope: `compat/baseline.toml` is the §5 pilot-gate baseline table, unrelated"); line 1091 (§7.4 "R1 correction" paragraph — reframed as authoritative "Corpus location" with explicit out-of-scope disambiguation for `compat/baseline.toml`); line 1093 (§7.4 follow-on paragraph — retitled "Corpus addition (R2)"); line 1176 (§7.4 trailing paragraph — retitled "Pilot-gate baseline (separate concern; NOT this PR)" to make the disambiguation header-explicit).
  - **H-3 fix (§10b duplicate row numbers).** R2 added rows 47-70 for the new test coverage, but two later validator rows still used row numbers 47 and 48 (duplicates). R3 renumbers the two validator rows to 71 and 72 with R3-tagged comments.
  - **B-2 leftover fix (`[workspace.package.include]` / `[workspace.package.exclude]` policy).** R2 §3.2.bis path policy table covered most workspace-root keys but omitted `[workspace.package.include]` and `[workspace.package.exclude]` (inherited path-glob arrays for `cargo package`). R3 adds two rows to the policy table classifying these as "Array of path globs / Verbatim" with a documented limitation note: `include`/`exclude` are PUBLISH-time concerns (not BUILD-time), the overlay's effective glob-resolution dir is `<overlay_root>` (with the staged-mirror per #40+#47 providing functionally-equivalent file sets for build purposes), and `cargo package` faithfulness is out of scope for v0.1.0 compat verdicts (which are build + test outcomes).

  R3 is a no-source-change revision (the implementer hasn't dispatched yet); all 18 cited line numbers from Codex R2 are addressed within the plan file only. Per [[sweep-after-review]] discipline: Codex R2's findings + B-2 leftover were sourced from a single Codex adversarial pass; R3 addresses ALL of them. The class-enumeration sweep was NOT re-run for R2 → R3 because R2's punch-list closure was thorough (R2 already addressed the R1 sweep's 30 punch items), and R3's surgical cleanup is bounded to internal-consistency fixes of R2's authoritative statements — no new class introductions. R3's bounded scope is verified by every touched line tracing back to a Codex R2 citation.
...
3. Test coverage: workspace-member entry shape added to the byte-determinism corpus at `tests/compat/overlay_corpus/` (new fixture pair `workspace_member_with_package.input.toml` + `workspace_member_with_package.expected.toml`; the corpus test at `tests/compat/overlay_determinism.rs:432-499` iterates the hardcoded names array). §7.4 corpus expansion. **Out of scope:** `compat/baseline.toml` is the §5 pilot-gate baseline table (per-pilot `n_max` ceilings, owned by `src/compat/gate.rs::parse_baseline`); it is UNRELATED to the overlay byte-determinism corpus and is not touched by #53.
...
| `[workspace.package.include]` | Array of path globs | Verbatim (carry array entries as-is; see notes) | R3 ADDED (Codex H-2 B-2 leftover). Cargo's `include`/`exclude` are PUBLISH-time globs used by `cargo package`, not BUILD-time concerns. For overlay dylib build, the array contents do not affect compile behavior. Carry the array entries verbatim so members inheriting `include = { workspace = true }` resolve the inheritance reference cleanly at overlay manifest-parse time. Globs are gitignore-style relative patterns; cargo's glob matcher resolves them against the manifest's directory. The overlay's effective glob-resolution dir is `<overlay_root>` (not `<workspace_root>`), so a glob like `src/**/*` resolves against the overlay-root's `src/` (which is symlinked / mirrored from the member's `src/` per the #40+#47 R8 staged-mirror plan), producing a functionally-equivalent file set for build purposes. Documented limitation: if an adopter relies on `cargo package` behavior matching across baseline vs overlay, the overlay does not faithfully reproduce that (out of scope for v0.1.0; compat verdicts are based on build + test outcomes, not package outcomes). |
| `[workspace.package.exclude]` | Array of path globs | Verbatim (carry array entries as-is; see notes above) | R3 ADDED (Codex H-2 B-2 leftover). Same shape and rationale as `[workspace.package.include]`. |
...
**R2 type-shape contract.** §3.1.bis defines a single `DualRoot` struct that covers BOTH the dual-root case (when `args.compat_package.is_some()`) and the collapsed single-root case (when `args.compat_package.is_none()`), using the explicit `workspace_member_context: Option<WorkspaceMemberContext>` field as the discriminator. (R1's earlier `ResolvedMember` struct + `UpstreamManifest` enum shapes are superseded — see the top-of-plan revision history R1 → R2 entry for the consolidation rationale.):
...
- Workspace inheritance tables (`[workspace.dependencies]`, etc.) captured into `WorkspaceMemberContext.workspace_root_value` for downstream carry-down.
...
- `[patch.crates-io]` — this is the #40+#47 territory. The Option H 4-rule policy (in the #40+#47 plan) handles `[patch.crates-io]` resolution; the ROOT's `[patch.crates-io]` table should be carried down to the overlay following the same rules. **§5 PRE-COMMITTED CONSTRAINT:** the implementer must merge the workspace ROOT's `[patch.crates-io]` table into the overlay using the same Option H 4-rule policy that #40+#47 use. The composition order is **root-first, member-second** (see §5.3.bis as the AUTHORITATIVE statement): the effective `[patch.crates-io]` table is computed from the workspace root FIRST (with path entries absolutized against `workspace_root` per §3.2.bis), THEN Option H's 4 rules run against that effective table with the member as the "self" target. A member-level `[patch.crates-io]` is a hard REJECT per §5.3.bis Step 2 (cargo itself errors on member `[patch]`). (Cargo's own behavior: workspace `[patch.crates-io]` is the only `[patch.crates-io]` cargo reads — members do not contribute. Verify against cargo reference: https://doc.rust-lang.org/cargo/reference/overriding-dependencies.html#the-patch-section. Cargo source: `crates/cargo/src/cargo/util/toml/mod.rs` for the `[patch]` discovery — members' `[patch]` tables are an error in cargo.) So in fact the implementer's job is simpler: ONLY the workspace ROOT's `[patch.crates-io]` is read; the member's is REJECTED if present.
...
R2 fixes this by specifying the composition order explicitly: **root-first, member-second.** §5.3.bis is the AUTHORITATIVE statement for this plan — any wording elsewhere (in §5.3 bullet-list, §6.2, the §7 tests, etc.) MUST match this section's order. The implementer follows this order verbatim.
...
3. **Step 3 — Run Option H's 4 rules against the effective workspace-root patch table (with the member as the "self" target).** This step runs AFTER Step 1's effective-table computation (root-first); the rules operate on the root-derived table, not on the member's own (rejected) `[patch.crates-io]`.
   - Rule 1 (INJECT) — if no `[patch.crates-io.<self>]` entry exists in the effective table (i.e. the workspace root did NOT declare a self-patch for the member), inject the synthetic self-patch `{ path = "<overlay_root>" }` per #40+#47 Option H Rule 1.
   - Rule 2 (REMAP) — if an existing `[patch.crates-io.<self>] = { path = "..." }` IS present in the effective table (i.e. the workspace root DID declare a self-patch on this member), re-anchor its `path` to the overlay-root form per Rule 2. The output of Rule 2 is overlay-root-relative, NOT workspace-root-relative — this is the key semantic that makes the staged-overlay-dir mirror behave as the self-patch target rather than the upstream's own fork path.
...
This is the #40+#47 territory. Pre-committed by §5.3 + §5.3.bis: the workspace root's `[patch.crates-io]` table is carried down to the overlay using the same Option H 4-rule policy that #40+#47 use. The implementer does NOT need to re-design the Option H policy — they extend the existing `apply_self_patch_policy` (the function the #40+#47 plan introduces; expected name) to take a workspace-root `[patch.crates-io]` table as an additional input. **The composition order is root-first, member-second per §5.3.bis (the AUTHORITATIVE statement):** the effective `[patch.crates-io]` table is computed from the workspace root FIRST (with path entries absolutized against `workspace_root`), THEN Option H's 4 rules run against the effective table with the member as the "self" target. A member-level `[patch.crates-io]` triggers the §5.3.bis Step 2 REJECT (cargo itself errors on member `[patch]`).

The order matters: cargo treats workspace `[patch]` as the only `[patch]` it reads (members' `[patch]` is an error). So in practice the workspace root's table is the only source, and lihaaf's resolver mirrors cargo by rejecting member-level `[patch]`.
...
10. `resolve_workspace_member_manifest_workspace_inheritance_captured` — verifies the resolver's returned tuple supplies `workspace_root_value` that the driver wraps into `DualRoot.workspace_member_context.workspace_root_manifest` + `workspace_root_value` for downstream use (per §4.6 R2 type-shape contract).
...
27. `option_h_root_first_member_second_with_workspace_root_self_patch_entry` — synthesizes a workspace where the WORKSPACE root declares `[patch.crates-io.pkg-name] = { path = "../local-fork" }` (an upstream self-patch on a member named `pkg-name`). Resolves `-p pkg-name` and asserts:
    - The overlay's top-level `[patch.crates-io.pkg-name].path` exists AND is REMAPPED to the staged-overlay-dir form per §5.3.bis Step 3 Rule 2 (REMAP). Specifically: because `pkg-name` is the member-under-test (the "self"), Rule 2 fires and re-anchors the workspace-root-declared `path = "../local-fork"` into the overlay-root form (`<member_root>/target/lihaaf-overlay/`-derived). The final path is NOT `<workspace_root>/../local-fork` (workspace-root-relative form) — that would be the Step 1 INTERMEDIATE form BEFORE Rule 2 runs; Rule 2's output supersedes it for the self entry. A non-self workspace-root patch entry (e.g. `[patch.crates-io.other-dep]`) would NOT trigger Rule 2 and WOULD remain Step-1-absolutized — verify on a test variant.
    - Option H Rule 1 (INJECT) does NOT inject a synthetic self-patch (the workspace-root entry already covers the member; the effective table has `[patch.crates-io.pkg-name]` populated, so Rule 1's "no self entry exists" precondition is false).
    - Option H Rule 2 (REMAP) IS applied to the workspace-root self-patch entry (the test asserts the final overlay path matches the overlay-root-derived form, not the workspace-root-derived intermediate). Test class: unit. Bites pre-fix: R1's underspecified composition order would either silently overwrite the workspace-root entry (Rule 1 INJECT-after-merge case) or shadow it (Rule 1 INJECT-before-merge case); R2's root-first order PLUS Rule 2 REMAP produces the correct merged table targeting the staged-overlay-dir.
...
**Corpus location (authoritative).** The byte-determinism corpus lives at `tests/compat/overlay_corpus/<name>.input.toml` + `<name>.expected.toml` (file-pair fixtures). The corpus test `byte_identical_across_two_lihaaf_binaries_on_corpus` at `tests/compat/overlay_determinism.rs:432-499` iterates a hardcoded `names = ["bare_package", "with_rlib_only", "with_cdylib", "with_patch_section", "with_comments", "with_replace_section"]` array (6 names) and asserts `checked == 6`. **Out of scope — DO NOT CONFUSE WITH `compat/baseline.toml`:** the file at `compat/baseline.toml` is the §5 pilot-gate baseline table (per-pilot `n_max` ceilings, owned by `src/compat/gate.rs::parse_baseline`) — UNRELATED to the overlay byte-determinism corpus. #53 does NOT touch `compat/baseline.toml`.

**Corpus addition (R2).** Add ONE new fixture pair to `tests/compat/overlay_corpus/`:
...
**Pilot-gate baseline (separate concern; NOT this PR).** The §5 pilot-gate baseline table at `compat/baseline.toml` (per-pilot `n_max` ceilings, owned by `src/compat/gate.rs::parse_baseline`) is UNRELATED to the byte-determinism corpus described above. axum-macros' pilot enrollment is a SEPARATE PR (per §7.6 — outside #53's scope); when that PR lands, the implementer adds an `[axum-macros]` table with `n_max = ...` to the pilot-gate `compat/baseline.toml`. R2 does NOT pre-commit that addition; #53 lands the CAPABILITY, the follow-up pilot-enrollment PR uses it.
...
| 10 | `resolve_workspace_member_manifest_workspace_inheritance_captured` | `grep -F 'fn resolve_workspace_member_manifest_workspace_inheritance_captured' src/compat/overlay.rs` | 1 | §7.2 #10 — resolver returns `workspace_root_value` for the driver to wrap into `DualRoot.workspace_member_context` |
...
| 71 | validator rule A: non-compat rejection (R3: renumbered from duplicate row 47) | `grep -F 'return Err(non_compat_mode_error("--package"));' src/cli.rs` | 1 | §3.3 rule A |
| 72 | validator rule B: mutual exclusion (R3: renumbered from duplicate row 48) | `grep -F 'cannot be combined: `--compat-manifest` supplies an explicit' src/cli.rs` | 1 | §3.3 rule B |
...
- **R3 (2026-05-18, post-Codex-R2-BLOCK)** — surgical cleanup per Codex R2 BLOCK-3 + 3 FIX_BEFORE_IMPL findings. See top-of-plan "Revision history" section for the full R2 → R3 delta map. Summary: BLOCK-3 (Option H composition order — 6 sites) aligned to §5.3.bis root-first authoritative statement (lines 697, 724, 739, 741, 824, 1011 — incl. test #27 assertion correction to match Rule 2 REMAP); H-1 (5 stale `ResolvedMember` references at lines 602, 646, 976, 1448, 1615) replaced with `DualRoot` / `WorkspaceMemberContext`; H-2 (4 stale `compat/baseline.toml` references at lines 73, 1091, 1093, 1176) replaced with real corpus path `tests/compat/overlay_corpus/` + explicit out-of-scope disambiguation; H-3 (§10b duplicate row numbers 47/48) renumbered to 71/72; B-2 leftover (`[workspace.package.include]` + `[workspace.package.exclude]` omission) addressed by adding two rows to §3.2.bis path policy table with documented limitation note. R3 is a no-source-change revision (the implementer hasn't dispatched yet); no new sections, no structural changes, no re-litigation of R2's closed BLOCKs.
...
- Resolver-return-shape struct vs tuple (R1 open item) → §3.1.bis closes this by consolidating into the `DualRoot` struct; the resolver returns `(PathBuf, toml::Value)` (a minimal tuple — just enough for the driver to construct a `DualRoot`), and `DualRoot` is the public API. (R1's earlier `ResolvedMember` shape is superseded.)
- Upstream-manifest enum vs flat struct (R1 open item) → §3.1.bis closes this by adopting the `DualRoot` struct (single shape, collapse via the `workspace_member_context: Option<_>` field). (R1's earlier `UpstreamManifest` enum is superseded.)
